...
Current focus: Sprint 3 COMPLETE (AR-1/2/3/4, SW-14, SE-10) + SW-15/SW-16 (dynamic funds, composite RD/FD goals) + QA-1 (real test suite). Next: Sprint 4 (AN-1, AN-5, SE-2) and the deferred follow-ups noted below.
...
| **AR-1b** | **Multi-device cloud-sync of corpus side-channel (instruments, per-fund rates, fund overlay)** | **P2 — Soon** | **Not Started** | 2 | — | — | S4 | SW-16 instruments + per-fund rates live in the `artha_goal_corpus` localStorage side-channel and SW-15 fund overlay in `artha_funds_v1`; these are NOT yet written to Supabase, so they survive a same-browser sign-out/in but NOT a browser wipe or second device. Extend cloud write-through + read-back (or the goals-table `config_json`) to cover them. |
...
- **Sprint 4 (Next):** AR-1b (multi-device cloud-sync of instruments/rates/fund overlay) + AN-1 + AN-5 + SE-2 — finish cloud durability, then signal trend detection, XIRR, rate limiting.
