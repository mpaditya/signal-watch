...
| **SW-15** | **Add / archive funds (no hard delete)** | **P2 — Soon** | **Not Started** | 1-2 | — | — | — | The fund universe (`FUNDS` in App.jsx) is currently hardcoded. As the builder churns their portfolio every 1–3 years (pauses some funds, adds new ones), they need to ADD new funds via the UI. Mirror the goal model: NO hard delete — only archive (soft-delete, data preserved) so historical signals/decisions referencing a fund stay intact. Persist the fund list (localStorage + Supabase `funds` table). Archived funds drop out of signal cards + goal fund-pickers but remain in history. Depends on AR-1 for cloud persistence. |
| **SW-16** | **Link RD/FD instruments to goals (not just MFs)** | **P2 — Soon** | **Not Started** | 1-2 | — | — | — | Goals can currently only be funded by mutual funds. Short-term goals are often funded partly by recurring deposits (RDs) / fixed deposits (FDs) — e.g. a car goal using an ongoing RD. Let the goal builder attach RD/FD instruments (principal, rate, maturity, monthly contribution for RDs) as corpus contributors so the projected-vs-target tracker and progress bar reflect them. RDs are ongoing (monthly inflow like a SIP but at a fixed rate); FDs are lump sums compounding at a fixed rate. Reuses goalUtils projection math with a fixed return instead of equity CAGR. Overlaps with AE-3 (fixed income tracker) and AE-4 (RD/FD instrument-level tracking) — this item is the goal-integration slice. |
