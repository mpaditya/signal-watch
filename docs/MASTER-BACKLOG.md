...
| **SW-13** | **Google Search grounding for chat panel** | **P1 — Next** | **Not Started** | 1-2 | — | — | S2-3 | Gemini's training data has a cutoff (~early 2025) so the chat returns stale info for "current" questions (Nifty P/E today, recent fund news, market events). Enable Gemini's native Google Search tool by adding `tools: [{ google_search: {} }]` to the API request. Free tier supports this. Returns citations. Keeps existing context injection (live NSE P/E etc.) as the primary source — search is a fallback for queries we don't pre-fetch. See DEC-043 (web search grounding for chat). |
