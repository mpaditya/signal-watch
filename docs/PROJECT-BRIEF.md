...
| Database | \*\*Supabase Postgres (free tier)\*\* | Sprint 3 (AR-1) + Sprint 4 (AR-1b). Tables: goals, config, signal\_history, decisions, user\_blobs. RLS enforced. REST API used directly (no SDK). Cloud sync of goals+config+corpus (read-back on login, debounced write-through). AR-1b: the `user_blobs` generic key-value table (DEC-053) now also syncs the `artha_goal_corpus` side-channel (per-fund rates, RD/FD instruments, legacy goalType/startDate/totalYears overrides) and the `artha_funds_v1` SW-15 fund overlay — full multi-device durability. |
